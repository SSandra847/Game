...
function Game(){
	...
	var high_score = game.querySelector("section#game ol.high-scores");
	...
	var gems = new Array();
	gem[0] = new Gem('green', 10, 1.2);
	gem[1] = new Gem('blue', 20, 1);
	gem[2] = new Gem('orange', 50, 0.75);
...
funcation UpdateScores(){
	if(typeof(Storage) !== "undefined"){
		...
		if(localStorage["high-scores"]){
			...
			scores[0] = current;
			localStorage["high-score"] = JSON.stringify(scores);
		}
		HighScores();
	}
}
